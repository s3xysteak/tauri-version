...
async function e2e() {
  ...
  return new Promise<void>((resolve) => {
    setTimeout(() => {
      exec('cd ./test/legend/__e2e__/src-tauri && cargo run')
    }, 1000)

    const child = exec('esno ./src/legend/cli.ts patch -b "test/legend/__e2e__" --no-git', async () => {
...
async function e2eVersionCheck() {
  return new Promise<void>((resolve) => {
    const child = exec('esno ./src/legend/cli.ts -b "test/legend/__e2e__"', async () => {
      const raw = await getVersions()
...
async function getVersions() {
  const packagePath = 'test/legend/__e2e__/package.json'
  const packageContent = await fs.readFile(packagePath, 'utf-8')

  const tauriConfPath = 'test/legend/__e2e__/src-tauri/tauri.conf.json'
  const tauriConf = await fs.readFile(tauriConfPath, 'utf-8')

  const tomlPath = 'test/legend/__e2e__/src-tauri/Cargo.toml'
  const toml = await fs.readFile(tomlPath, 'utf-8')

  const lockPath = 'test/legend/__e2e__/src-tauri/Cargo.lock'
